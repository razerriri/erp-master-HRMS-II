...
public class EJCDao {
	...
	public List<Competency> getEmployeeJobCompetency(Competency c){
		return template.query("SELECT (TBL_JOB.JOB_NAME+' - '+TBL_DEPARTMENT.DEPT_NAME),  "
				...
				+" FROM   TBL_JOB INNER JOIN "
				+" TBL_JOB_COMPETENCY ON TBL_JOB.JOB_ID = TBL_JOB_COMPETENCY.JOB_COMPETENCY_JOB_ID INNER JOIN "
				...
				+" TBL_COMPETENCY_STATUS ON TBL_COMPETENCY.COMPETENCY_ID = TBL_COMPETENCY_STATUS.COMPETENCY_STATUS_COMPETENCY_ID CROSS JOIN "
				+" TBL_EMPLOYEE INNER JOIN "
				+" TBL_DEPARTMENT ON TBL_JOB.DEPT_ID = TBL_DEPARTMENT.DEPT_ID "
				+ "WHERE TBL_EMPLOYEE.EMP_ID='"+c.getEmployee_id()+"' AND TBL_JOB.JOB_ID='"+c.getJob_id()+"'", new RowMapper<Competency>(){
			public Competency mapRow(ResultSet rs, int row)  throws SQLException{
	...
	public List<Competency> getEmployeeJobCompetencyindex(){
		return template.query("SELECT (TBL_JOB.JOB_NAME+' - '+TBL_DEPARTMENT.DEPT_NAME),  "
				...
				+" TBL_COMPETENCY_STATUS ON TBL_COMPETENCY.COMPETENCY_ID = TBL_COMPETENCY_STATUS.COMPETENCY_STATUS_COMPETENCY_ID CROSS JOIN "
				+" TBL_EMPLOYEE INNER JOIN "
				+" TBL_DEPARTMENT ON TBL_JOB.DEPT_ID = TBL_DEPARTMENT.DEPT_ID ", new RowMapper<Competency>(){
			public Competency mapRow(ResultSet rs, int row)  throws SQLException{
				Competency c = new Competency();
				c.setJob_name(rs.getString(1));
				c.setCompetency_name(rs.getString(2));
