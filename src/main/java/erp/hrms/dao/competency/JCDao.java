...
+ "AND JOB_COMPETENCY_COMPETENCY_LEVEL!='Basic'  AND JOB_COMPETENCY_COMPETENCY_LEVEL!='Intermediate' THEN  'Needs Improvement' ELSE 'Desired Level Attained' END AS SCORE   "
+ "FROM TBL_JOB    "
...
+ "INNER JOIN TBL_COMPETENCY ON TBL_JOB_COMPETENCY.JOB_COMPETENCY_COMPETENCY_ID = TBL_COMPETENCY.COMPETENCY_ID "
+ "INNER JOIN TBL_DEPARTMENT ON TBL_JOB.DEPT_ID = TBL_DEPARTMENT.DEPT_ID WHERE TBL_EMPLOYEE.EMP_ID="+empid+" AND TBL_JOB.JOB_ID="+jobid+"", new RowMapper<Competency>(){
			public Competency mapRow(ResultSet rs, int row)  throws SQLException{
